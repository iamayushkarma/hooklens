...
export default function IntegrationsCard({ style }: { style: string }) {
  ...
  const iconRef = useRef<HTMLDivElement>(null);
  const badgeRefs = useRef<Record<string, HTMLDivElement | null>>({});
  const { active: isHovered, handlers } = useCardInteraction();
  const [lines, setLines] = useState<
    { x1: number; y1: number; x2: number; y2: number }[]
  >([]);
  ...
  const badges = [
    {
      label: "Stripe",
      style: "top-4 left-2 sm:top-6 sm:left-10 md:top-9 md:left-22",
      icon: <BsStripe />,
    },
    {
      label: "GitHub",
      style: "top-4 right-2 sm:top-6 sm:right-16 md:top-9 md:right-43",
      icon: <FaGithub />,
    },
    {
      label: "Slack",
      style: "bottom-3 left-3 sm:bottom-5 sm:left-16 md:bottom-6 md:left-40",
      icon: <FaSlack />,
    },
    {
      label: "Twilio",
      style: "bottom-2 right-2 sm:bottom-3 sm:right-10 md:bottom-4 md:right-25",
      icon: <TbBrandTwilio />,
    },
  ];
  ...
  return (
    ...
    >
      <motion.div
        aria-hidden
        className="pointer-events-none absolute! left-1/2 top-1/3 h-32 w-48 sm:h-40 sm:w-56 md:h-44 md:w-64 -translate-x-1/2 -translate-y-1/2 rounded-full blur-[50px]"
        style={{
          background:
            "radial-gradient(closest-side, rgba(37,99,235,0.5), rgba(37,99,235,0) 70%)",
        }}
      ...
      <motion.div
        aria-hidden
        className="pointer-events-none absolute -right-8 top-4 h-36 w-36 sm:h-44 sm:w-44 md:-right-12 md:h-52 md:w-52 rounded-full bg-danger-bg blur-[80px]"
        initial={false}
        animate={{
          opacity: isHovered ? 0.8 : 0,
          scale: isHovered ? 1.1 : 0.75,
      ...
      <motion.div
        aria-hidden
        className="pointer-events-none absolute left-1/3 top-10 h-28 w-44 sm:top-14 sm:h-36 sm:w-56 md:top-16 md:h-40 md:w-64 rounded-full bg-warning-bg blur-[90px]"
        initial={false}
        animate={{
          opacity: isHovered ? 0.5 : 0,
          scale: isHovered ? 1.05 : 0.8,
      ...
      <div
        ref={containerRef}
        className="relative h-40 sm:h-48 md:h-52 flex items-center justify-center group-hover:scale-[1.04] transition-all duration-500 ease-in-out select-none"
      >
        <svg className="absolute inset-0 w-full h-full pointer-events-none">
          {lines.map((line, i) => {
            return (
        ...
        <div
          ref={iconRef}
          className="size-9 sm:size-10 md:size-11 flex items-center justify-center rounded-sm relative z-10 transition-shadow duration-200"
        >
          <img src={HookLensLogo} className="size-10 sm:size-11 md:size-12" />
        </div>
...
const BadgePill = ({
  ...
  return (
    <div
      ref={refCallback}
      className={`${style} flex items-center justify-between gap-1 sm:gap-1.5 bg-white rounded-lg px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2 absolute border border-border-default shadow-md z-10 text-xs sm:text-sm whitespace-nowrap`}
    >
      {icon}
      {label}
    </div>
