...
export default function WriteBetterCard() {
  ...
  return (
    ...
    >
      {/* explicit-height wrapper so absolute children (glow, canvas) can
          actually fill the full card interior instead of collapsing to
          the editor panel's natural size. CardLayout's card has a fixed
          height (h-82) with p-6 padding, so h-full here resolves against
          that definite, already-known content height. */}
      <div className="relative h-full w-full">
        ...
        {/* confetti canvas — now spans the full card interior, not just
            the editor panel's box */}
        <canvas
          ref={canvasRef}
