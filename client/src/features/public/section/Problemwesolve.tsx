...
function ProblemCard({ item }: { item: (typeof items)[number] }) {
  ...
  return (
    <div className="border-r border-b border-border-default p-6 sm:p-8 md:p-10 lg:p-12 hover:bg-gray-50/80">
      <Icon
        size={20}
        strokeWidth={1.5}
        className="mb-4 sm:mb-6 text-text-secondary"
      />
      <h3 className="m-0 mb-2 sm:mb-3 font-mono text-[11px] sm:text-[12px] font-medium uppercase tracking-wider text-text-primary">
        {item.title}
      </h3>
      <p className="m-0 max-w-[26ch] text-[13px] sm:text-[14px] leading-relaxed text-text-secondary">
        {item.body}
      </p>
    </div>
  );
...
export default function ProblemWeSolve() {
  return (
    <section className="bg-white px-4 sm:px-5 py-16 sm:py-20 md:py-28">
      <div className="mx-auto max-w-xl text-center">
        <h2 className="m-0 mb-3 text-[26px] sm:text-[30px] md:text-[34px] lg:text-[42px] font-bold leading-tight text-text-primary">
          Webhooks are a black box until something breaks
        </h2>
        <p className="m-0 text-[14px] sm:text-[15px] text-text-secondary">
          Stripe, GitHub, Twilio they all fire events at you blind. Here's what
          that actually costs.
        </p>
      </div>

      <div className="mx-auto mt-10 sm:mt-14 grid max-w-4xl grid-cols-1 border-t border-l border-border-default sm:grid-cols-2 md:grid-cols-3">
        {items.map((item) => (
